...
class EstateStatusController @Inject() (
  ...
  def checkStatus(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { utr =>
      estateStoreConnector.get(utr).flatMap {
        case Some(lock) if lock.estateLocked =>
          logger.info(
            s"[Session ID: ${Session.id(hc)}][UTR: $utr]  $utr user has failed IV 3 times, locked out for 30 minutes"
          )
  ...
  def inProcessing(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { utr =>
      Future.successful(Ok(inProcessingView(utr, isAgentUser)))
    }
  }

  def closed(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { utr =>
      Future.successful(Ok(closedView(utr, isAgentUser)))
    }
  }

  def utrDoesNotMatchRecords(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { _ =>
      Future.successful(Ok(utrDoesNotMatchRecordsView(isAgentUser)))
    }
  }

  def problemWithService(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { _ =>
      Future.successful(Ok(problemWithServiceView(isAgentUser)))
    }
  }

  def accountNotLinked(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { utr =>
      Future.successful(Ok(accountNotLinkedView(utr)))
    }
  }

  def locked(): Action[AnyContent] = actions.authWithData.async { implicit request =>
    enforceUtr() { utr =>
  ...
  private def tryToPlayback(utr: String)(implicit request: DataRequest[AnyContent]): Future[Result] = {
    ...
    connector.getEstate(utr) flatMap {
      ...
      case Processing           =>
        logger.info(
          s"[Session ID: ${Session.id(hc)}][UTR: $utr] $utr unable to retrieve estate due it being in processing"
        )
      ...
      case UtrNotFound          =>
        logger.info(
          s"[Session ID: ${Session.id(hc)}][UTR: $utr] $utr unable to retrieve estate due to UTR not being found"
        )
